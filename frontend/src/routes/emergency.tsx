  ...
  head: () => ({
    meta: [
      { title: "Emergency Blocking | IR-ABPS" },
      {
        name: "description",
        content:
          "Initiate emergency disconnections and view active crisis blocks across the corridor.",
      },
    ],
  }),
...
const INITIAL_ACTIVITY: ActivityEvent[] = [
  {
    ...
    text: "Track fracture reported",
    detail: "CNB Outer",
  },
  {
    id: "e2",
    time: new Date(Date.now() - 24 * 60000),
    text: "Emergency block initiated",
    detail: "SOS-CNB-001",
  },
  {
    id: "e3",
    time: new Date(Date.now() - 23 * 60000),
    text: "Control notified",
    detail: "Automated alert sent",
  },
  {
    id: "e4",
    time: new Date(Date.now() - 22 * 60000),
    text: "Traffic protection confirmed",
    detail: "Signals set to DANGER",
  },
  {
    id: "e5",
    time: new Date(Date.now() - 10 * 60000),
    text: "OHE Snapping detected",
    detail: "NDLS - GZB",
  },
  {
    id: "e6",
  ...
  const confirmInitiateSOS = () => {
    ...
    setTimeout(() => {
      ...
      const newEmergency: Emergency = {
        ...
        severity:
          newType.includes("Fracture") || newType.includes("Disaster") || newType.includes("Hazard")
            ? "CRITICAL"
            : "HIGH",
        startedAt: new Date(),
        controlNotified: true,
  ...
  return (
    <>
      ...
      <div className="mb-8 grid gap-4 grid-cols-2 lg:grid-cols-4">
        ...
        <Card
          className={`shadow-sm border-border ${criticalCount > 0 ? "bg-destructive/10" : "bg-secondary/5"}`}
        >
          <CardContent className="p-4 flex items-center justify-between">
            <div>
              <p className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground mb-1">
              ...
              <p
                className={`text-2xl font-mono font-bold ${criticalCount > 0 ? "text-destructive" : "text-foreground"}`}
              >
                {criticalCount}
              </p>
            </div>
            <div
              className={`p-2 rounded-full ${criticalCount > 0 ? "bg-destructive/20 text-destructive" : "bg-secondary text-muted-foreground"}`}
            >
        ...
        <Card className="shadow-sm bg-secondary/5 border-border">
          <CardContent className="p-4 flex items-center justify-between">
            <div>
              ...
              <p
                className={`text-xl font-bold ${emergencies.length > 0 ? "text-blue-500" : "text-muted-foreground"}`}
              >
                {emergencies.length > 0 ? "ACTIVE" : "STANDBY"}
              </p>
            </div>
            <div
              className={`p-2 rounded-full ${emergencies.length > 0 ? "bg-blue-500/10 text-blue-500" : "bg-secondary text-muted-foreground"}`}
            >
      ...
      >
        ...
        <CardContent className="p-6">
          ...
          <div className="mt-5 flex items-center gap-2 text-xs text-destructive/80 bg-destructive/10 p-2.5 rounded border border-destructive/20">
            ...
            <p>
              <strong>Warning:</strong> Emergency blocks immediately protect the selected section,
              set signals to danger, and notify central control.
            </p>
          </div>
        </CardContent>
      </Card>
          ...
          {emergencies.length === 0 ? (
            <div className="flex flex-col items-center justify-center p-12 text-center border border-border border-dashed rounded-lg bg-secondary/5">
              ...
              <p className="text-sm text-muted-foreground mt-1">
                Corridor currently operating under normal emergency status.
              </p>
            </div>
          ) : (
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2">
              {emergencies.map((emg) => (
                <Card
                  key={emg.id}
                  className={`shadow-sm border-t-4 overflow-hidden flex flex-col ${getSeverityBadge(emg.severity).replace("text-", "border-t-").replace("bg-", "border-t-").split(" ")[0]}`}
                >
                  <CardHeader className="bg-secondary/10 p-4 pb-3">
                    <div className="flex items-center justify-between mb-2">
                      <Badge
                        variant="outline"
                        className={`text-[10px] font-bold tracking-wider ${getSeverityBadge(emg.severity)}`}
                      >
                        {emg.severity}
                      </Badge>
                      <span className="text-xs font-mono text-muted-foreground">{emg.id}</span>
                    </div>
                    <CardTitle className="text-base font-bold truncate" title={emg.type}>
                      {emg.type}
                    </CardTitle>
                    <CardDescription
                      className="text-xs flex items-center gap-1.5 mt-1 truncate"
                      title={`${emg.section} · ${emg.line}`}
                    >
                  ...
                  <CardContent className="p-4 pt-3 flex-1 flex flex-col justify-between">
                    <div className="space-y-3 mb-4">
                      <div className="flex justify-between items-center text-xs">
                        ...
                        <span className="font-medium flex items-center gap-1">
                          <Clock className="size-3" /> {getTimeAgo(emg.startedAt)}
                        </span>
                      </div>
                      <div className="flex justify-between items-center text-xs">
                        <span className="text-muted-foreground uppercase font-semibold text-[10px] tracking-wider">
                        ...
                        <span className={`font-bold ${getSeverityColor(emg.severity)}`}>
                          {emg.severity}
                        </span>
                      </div>
                      <div className="flex justify-between items-center text-xs">
                        ...
                        <span className="font-bold text-safe flex items-center gap-1">
                          <RadioTower className="size-3" /> NOTIFIED
                        </span>
                      </div>
                      <div className="flex justify-between items-center text-xs">
                        <span className="text-muted-foreground uppercase font-semibold text-[10px] tracking-wider">
                    ...
                    <div className="grid grid-cols-2 gap-2 mt-auto">
                      <Button
                        ...
                        onClick={() => setDetailsDrawer(emg.id)}
                      >
                        VIEW DETAILS
                      </Button>
                      <Button
                        ...
                        onClick={() => setResolveModal(emg.id)}
                      >
                        RESOLVE BLOCK
                      </Button>
                    </div>
            ...
            <CardContent className="p-4 pt-0 space-y-2.5">
              <div className="flex items-start gap-2 text-xs">
                ...
                <span className="text-muted-foreground">
                  Verify affected section before initiating a block.
                </span>
              </div>
              <div className="flex items-start gap-2 text-xs">
                <CheckCircle2 className="size-3.5 text-blue-500 shrink-0 mt-0.5" />
                <span className="text-muted-foreground">
                  Emergency block triggers control-room notification.
                </span>
              </div>
              <div className="flex items-start gap-2 text-xs">
                <CheckCircle2 className="size-3.5 text-blue-500 shrink-0 mt-0.5" />
                <span className="text-muted-foreground">
                  Maintain protection until authorized field clearance.
                </span>
              </div>
            </CardContent>
          </Card>
        ...
        <DialogContent className="sm:max-w-md border-destructive/30 bg-card">
          <DialogHeader>
            ...
            <DialogDescription className="text-foreground pt-2">
              You're about to initiate an emergency traffic block. All signals in the affected zone
              will immediately be set to danger.
            </DialogDescription>
          </DialogHeader>
        ...
        <DialogContent className="sm:max-w-md border-safe/30 bg-card">
          ...
          <div className="py-2">
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Resolving this block will notify control to resume normal operations.
            </p>
          </div>
      ...
      <Sheet open={!!detailsDrawer} onOpenChange={(o) => !o && setDetailsDrawer(null)}>
        <SheetContent className="w-full overflow-y-auto sm:max-w-md border-l border-border">
          <SheetHeader className="border-b border-border/50 pb-4 mb-4 text-left">
            <Badge
              variant="outline"
              className={`w-fit mb-2 text-[10px] font-bold tracking-wider ${activeDrawerBlock ? getSeverityBadge(activeDrawerBlock.severity) : ""}`}
            >
            ...
            <div className="space-y-6 pb-8">
              <div className="grid grid-cols-2 gap-y-4 gap-x-2 text-sm bg-secondary/10 p-4 rounded-lg border border-border/50">
                ...
                <div>
                  ...
                  <p className="font-medium text-foreground">
                    {getTimeAgo(activeDrawerBlock.startedAt).replace(" ago", "")}
                  </p>
                </div>
              </div>
              ...
              <div className="bg-blue-500/10 border border-blue-500/20 p-3 rounded-lg flex gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-foreground">
                    Maintain emergency block until field clearance is received.
                  </p>
                </div>
              </div>
...
function TimelineNode({
  ...
  return (
    <div className="flex gap-3 relative">
      {!isLast && (
        <div
          className={`absolute left-[7px] top-5 bottom-[-16px] w-[2px] ${active ? "bg-primary" : "bg-border"}`}
        />
      )}
      <div
        className={`mt-0.5 size-4 shrink-0 rounded-full border-2 bg-background flex items-center justify-center z-10 ${active ? "border-primary" : "border-muted-foreground"}`}
      >
