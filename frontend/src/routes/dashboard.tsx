              ...
              <div className="divide-y divide-border">
                {data?.corridor_status.map((c) => {
                  ...
                  if (c.traffic_intensity > 70) {
                    intensityColor = "bg-warn";
                    statusLabel = "Congested";
                  }
                  if (c.traffic_intensity > 90) {
                    intensityColor = "bg-destructive";
                    ...
                    <div key={c.id} className="group p-5 transition-colors hover:bg-secondary/30">
                      <div className="flex flex-wrap items-start justify-between gap-4">
                        <div>
                          ...
                          <div className="mt-1.5 flex items-center gap-3 text-xs text-muted-foreground">
                            ...
                            <span>·</span>
                            <span>
                              <strong className="text-foreground">{c.trains_running}</strong> trains
                              running
                            </span>
                            <span>·</span>
                      ...
                      <div className="mt-4">
                        <div className="mb-1.5 flex justify-between text-xs font-medium">
                          ...
                          <span
                            className={
                              c.traffic_intensity > 90 ? "text-destructive" : "text-foreground"
                            }
                          >
                            {c.traffic_intensity}%
                          </span>
                ...
                {data?.urgent_risks
                  ...
                  .map((r) => {
                    ...
                    if (r.severity === "Critical") {
                      sevColor = "bg-destructive/10 text-destructive border-destructive/20";
                      iconColor = "text-destructive";
                    }
                    if (r.severity === "High") {
                      sevColor = "bg-warn/10 text-warn border-warn/20";
                      iconColor = "text-warn";
                    }
                    if (r.severity === "Medium") {
                      sevColor = "bg-yellow-500/10 text-yellow-600 border-yellow-500/20";
                    ...
                    return (
                      <div key={r.id} className="p-4 transition-colors hover:bg-secondary/20">
                        <div className="flex items-start justify-between gap-2">
                          ...
                          <Badge
                            variant="outline"
                            className={`h-5 border text-[10px] uppercase font-bold tracking-wider ${sevColor}`}
                          >
                            {r.severity}
                          </Badge>
                        </div>
            ...
            {loading ? (
              ...
              <div className="divide-y divide-border">
                {data?.train_forecast.map((t) => {
                  ...
                  if (t.status === "Delayed")
                    statusColor = "bg-destructive/15 text-destructive border-destructive/20";

                  return (
                    <div
                      key={t.id}
                      className="flex items-center justify-between p-3 px-5 transition-colors hover:bg-secondary/20"
                    >
                      <div className="flex items-center gap-4">
                        <div className="w-20 font-mono text-sm font-bold text-foreground">
                          {t.train}
                        </div>
                      ...
                      <Badge
                        variant="outline"
                        className={`font-semibold tracking-wider text-[10px] uppercase h-5 ${statusColor}`}
                      >
                        {t.status === "On Time"
                          ? "● ON TIME"
                          : t.status === "Expected"
                ...
                {[
                  ...
                  return (
                    <div key={item.key} className="flex items-center gap-4">
                      ...
                      <span className="w-8 text-right font-mono text-sm font-bold">{val}</span>
                    </div>
                  );
                })}
                <div className="pt-2">
          ...
          <CardContent className="p-5">
            {loading ? (
              ...
              <div className="py-6 text-center text-muted-foreground">
                No urgent operational priorities.
              </div>
            ) : (
              <div className="space-y-5">
                {data?.urgent_risks.slice(0, 3).map((r, i) => {
                  ...
                  if (r.severity === "Critical")
                    sevColor = "bg-destructive text-destructive-foreground";
                  if (r.severity === "High") sevColor = "bg-warn text-warn-foreground";

                  return (
                    <div key={r.id} className="flex gap-3">
                      <span
                        className={`flex size-6 shrink-0 items-center justify-center rounded-full text-xs font-bold ${sevColor}`}
                      >
                        {i + 1}
                      </span>
                      <div>
                        <h4 className="text-sm font-semibold text-foreground leading-none mb-1.5">
                          {r.title}
                        </h4>
                        <p className="text-xs text-muted-foreground flex items-center gap-1.5">
                          <span
                            className={`font-medium ${r.severity === "Critical" ? "text-destructive" : r.severity === "High" ? "text-warn" : ""}`}
                          >
                            {r.severity}
                          </span>
                          <span>·</span>
          ...
          <div className="flex flex-wrap gap-4 md:gap-8">
            {["COA", "BDMS", "TMS", "SMMS", "TDMS", "AI Planning"].map((sys) => (
              <div key={sys} className="flex items-center gap-2">
                ...
                <span className="text-[11px] text-muted-foreground hidden sm:inline uppercase tracking-wider">
                  Integrated
                </span>
              </div>
            ))}
